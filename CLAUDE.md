...
현재 작업 브랜치: `phase-0/project-setup`
...
현재 Phase의 상세 가이드만 이 파일에 포함한다. 이후 Phase는 `docs/specs/phase-N-<name>.md` 참조.

### Phase 1: B+Tree 구현 가이드 (현재 진행 중)
...
- Key 타입: `ByteArray` (가변 길이, length-prefixed 직렬화). 비교는 lexicographic order.
- Value 타입: `ByteArray` (가변 길이, length-prefixed 직렬화).
- Phase 1에서는 편의를 위해 `String` 키/값을 UTF-8 인코딩하여 사용한다.
