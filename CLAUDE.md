...
- **기능을 만들기 전에 기존 RDB가 어떻게 구현했는지 먼저 조사한다.** PostgreSQL을
  주 참조로 하되, MySQL/SQLite 등 다른 DB가 더 좋은 비교 대상이면 함께 본다.
...
│       │   └── server/        ← Phase 8: GwanServer, ConnectionHandler, PgMessage, Wire Protocol
...
| Constraints & Error Semantics | 트랜잭션 실패 상태 (25P02) | `txn/TransactionAbortedException.kt`, `txn/DatabaseSession.kt` (`txnStatus`, `executeInFailedTxn`) |
...
6. 고도화 항목 완료 시 `docs/specs/advanced.md`에 ✅ 표시와 결정 이유·**참고 자료** 기록
